...
namespace FMCOLOR
{
    ...
    public enum FMColorMode { FMCOLOR_FULL, FMCOLOR_FAST, FMCOLOR_URP }

    [ExecuteInEditMode]
    [RequireComponent(typeof(Camera))]
    public class FMColor : MonoBehaviour
    {
        public FMColorMode Mode = FMColorMode.FMCOLOR_FULL;
        ...
        private void Start()
        {
            ...
            if (Mode == FMColorMode.FMCOLOR_FULL)
            {
        ...
        private IEnumerator EndFrameBlit()
        {
            yield return new WaitForEndOfFrame();
            FMCOLOR_Blit();
        }
        private void FMCOLOR_Blit()
        {
            UpdateMaterialSettings();
            ...
            {
                //clone all pass, and target eyes for vr..etc
                RenderTextureDescriptor sourceDescriptor = UnityEngine.XR.XRSettings.enabled ? UnityEngine.XR.XRSettings.eyeTextureDesc : new RenderTextureDescriptor(rt.width, rt.height, rt.format);
                sourceDescriptor.depthBufferBits = 16;
                sourceDescriptor.sRGB = true;

                RenderTexture MainTex = RenderTexture.GetTemporary(sourceDescriptor);
                Graphics.Blit(rt, MainTex, material, pass);
                Graphics.Blit(MainTex, null as RenderTexture, FXAAMat);
                RenderTexture.ReleaseTemporary(MainTex);
            }
            else
            {
                Graphics.Blit(rt, null as RenderTexture, material, pass);
            }
        }
        ...
        private void CheckResolution()
        {
            ...
            _Resolution = new Vector2(Screen.width / (1 + PaintBlurLevel), Screen.height / (1 + PaintBlurLevel));
        
            //clone all pass, and target eyes for vr..etc
            RenderTextureDescriptor sourceDescriptor = UnityEngine.XR.XRSettings.enabled ? UnityEngine.XR.XRSettings.eyeTextureDesc : new RenderTextureDescriptor(Screen.width, Screen.height, RenderTextureFormat.ARGB32);
            sourceDescriptor.depthBufferBits = 16;
            sourceDescriptor.sRGB = true;
            sourceDescriptor.width = Mathf.RoundToInt(_Resolution.x);
            sourceDescriptor.height = Mathf.RoundToInt(_Resolution.y);

            if (rt == null)
            {
                rt = new RenderTexture(sourceDescriptor);
        ...
        private void OnRenderImage(RenderTexture source, RenderTexture destination)
        {
            ...
            else
            {
                ...
                if (IgnoreSkybox || FogSettings.Contribution > 0 || FresnelSettings.Contribution > 0 || OutlineSettings.Contribution > 0)
                {
                ...
                int depth = IgnoreSkybox || FogSettings.Contribution > 0 || FresnelSettings.Contribution > 0 || OutlineSettings.Contribution > 0 ? 16 : 0;
                int pass = (QualitySettings.activeColorSpace == ColorSpace.Linear ? 1 : 0) + ((PaintBlurLevel > 0 || PaintRadius > 0) ? 2 : 0);

                //clone all pass, and target eyes for vr..etc
                RenderTextureDescriptor sourceDescriptor = UnityEngine.XR.XRSettings.enabled ? UnityEngine.XR.XRSettings.eyeTextureDesc : new RenderTextureDescriptor(Screen.width, Screen.height, source.format);
                sourceDescriptor.depthBufferBits = depth;
                sourceDescriptor.sRGB = true;
                sourceDescriptor.width = source.width / (1 + PaintBlurLevel);
                sourceDescriptor.height = source.height / (1 + PaintBlurLevel);

                if (PaintBlurLevel > 0)
                {
                    RenderTexture RTexture = RenderTexture.GetTemporary(sourceDescriptor);
                    RTexture.filterMode = FilterMode.Trilinear;
        ...
        private string TargetShaderName = "Hidden/FMColor";
        public void UpdateMaterialSettings()
        {
            switch (Mode)
            {
                case FMColorMode.FMCOLOR_FULL: TargetShaderName = "Hidden/FMColor"; break;
                case FMColorMode.FMCOLOR_FAST: TargetShaderName = "Hidden/FMColorFast"; break;
                case FMColorMode.FMCOLOR_URP: TargetShaderName = "Hidden/FMColorURP"; break;
            }

            if (material == null)
            {
                material = new Material(Shader.Find(TargetShaderName));
                //material.hideFlags = HideFlags.HideAndDontSave;
            }
            else
            {
                if (material.shader.name != TargetShaderName) material = new Material(Shader.Find(TargetShaderName));
            }
        ...
        private void SetMaterial(Material _mat)
        {
            ...
            //Flip
            _mat.SetFloat("_HorizontalFlip", HorizontalFlip ? 1f : 0f);
            _mat.SetFloat("_VerticalFlip", VerticalFlip ? 1f : 0f);
            ...
            _mat.SetFloat("_IgnoreSkybox", IgnoreSkybox ? 1f : 0f);
        }

        private IEnumerator DebugSliderLerp(float _value)
        {
            AllowSlide = false;
        ...
        public void Action_ToggleFXAA(bool _value) { FXAASettings.Enable = _value; }
        #endregion
    }
}