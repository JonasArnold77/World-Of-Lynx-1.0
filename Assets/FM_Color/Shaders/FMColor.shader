Shader "Hidden/FMColor"
{
    ...
    uniform UNITY_DECLARE_SCREENSPACE_TEXTURE(_MainTex); //for single pass vr
    //uniform sampler2D _MainTex;
    uniform half4 _MainTex_TexelSize;

    
    uniform UNITY_DECLARE_SCREENSPACE_TEXTURE(_MainTexStencil);
    ...
    uniform UNITY_DECLARE_SCREENSPACE_TEXTURE(_CameraDepthTexture);
    ...
    {
        half4 depthnormal = UNITY_SAMPLE_SCREENSPACE_TEXTURE(_CameraDepthNormalsTexture, uv);
        half3 normal;
        half depth;
    ...
    inline half3 ApplyPaintEffect(float2 uv)
    {
        ...
        for (int k = 0; k < 4; k++) 
        {
            for(int i = 0; i <= _PaintRadius; i++) 
            {
                for(int j = 0; j <= _PaintRadius; j++) 
                {
                    ...
                    col = UNITY_SAMPLE_SCREENSPACE_TEXTURE(_MainTex, float2(uv + float2(pos.x * _MainTex_TexelSize.x, pos.y * _MainTex_TexelSize.y))).rgb;
        ...
        //if(GetLuma(c) < GetLuma(UNITY_SAMPLE_SCREENSPACE_TEXTURE(_MainTex, uv))) c *=0.8 * (GetLuma(UNITY_SAMPLE_SCREENSPACE_TEXTURE(_MainTex, uv)) * 0.5 + 0.5);
        //if((GetLuma(c) - GetLuma(UNITY_SAMPLE_SCREENSPACE_TEXTURE(_MainTex, uv))) * (GetLuma(c) - GetLuma(UNITY_SAMPLE_SCREENSPACE_TEXTURE(_MainTex, uv))) > 0.0001) c*=0.5;
    ...
    inline half3 ApplyIgnoreSkybox(half3 c, float2 uv)
    {
        return IF(GetNormalDepth(uv).w >= 1, UNITY_SAMPLE_SCREENSPACE_TEXTURE(_MainTex, uv).rgb, c);
    }
    ...
    fixed4 frag(v2f i) : SV_Target
    {
        ...
        half4 c = UNITY_SAMPLE_SCREENSPACE_TEXTURE(_MainTex, uv);
        ...
        c.rgb = IF(i.uv.x < _DebugSlider, ApplyDebug(UNITY_SAMPLE_SCREENSPACE_TEXTURE(_MainTex, i.uv).rgb, i.uv.x), c);
        c.rgb = IF(_IgnoreSkybox == 1, ApplyIgnoreSkybox(c, uv).rgb, c);
        return c;
    }
